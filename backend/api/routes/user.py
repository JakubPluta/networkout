...
@router.put('/{user_id}',  response_model=schemas.UserFromDB, status_code=status.HTTP_201_CREATED)
...
@router.delete('/{user_id}',  response_model=schemas.UserFromDB, status_code=status.HTTP_200_OK)
...
def befriend(*, db: Session = Depends(get_db), current_user: User = Depends(get_current_user), other_user_id: int):
    ...
    try:
        ...
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail=f"{str(e)}") from e
    return user
...
@router.get('/{user_id}/friends/', status_code=status.HTTP_201_CREATED, response_model=schemas.Friends)
def get_user_friends(*, user_id: int, db: Session = Depends(get_db), superuser: User = Depends(get_current_superuser)):
    user = user_crud.get_user(db, user_id)
    if user is None:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail=f"User with id {user_id} not found")
