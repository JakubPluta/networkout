...
@router.get(
    "/{role_name}/name", response_model=schemas.RoleDB, status_code=status.HTTP_200_OK
)
...
def create_role(
    *,
    ...
    if role_db is not None:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
            detail=f"Role with name {role.name}already exists in database",
        )
    return role_crud.create_role(db, role)


@router.put(
    "/{role_id}", response_model=schemas.RoleDB, status_code=status.HTTP_201_CREATED
)
def update_role(
    *,
    role_id: int,
...
def delete_role(
    ...
    return {
        "msg": f"Role with id:{role_id} and name: {role_db.name} successfully deleted"
    }
