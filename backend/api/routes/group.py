...
@router.delete('/{group_id}', response_model=schemas.GroupDB, status_code=status.HTTP_200_OK)
def delete_group(group_id: int, db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
    ...
    raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail="You don't have permission to delete group with id {group_id}")
...
def remove_user_from_group(*, group_id: int, user_id: int, db: Session = Depends(get_db),
    ...
    try:
        ...
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail=f"User with {user_id} doesn't belong to group {group_ud}")
    return group
